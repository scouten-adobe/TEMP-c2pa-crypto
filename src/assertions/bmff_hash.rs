...
/// Helper class to create BmffHash assertion. (These are auto-generated by the
/// SDK.)
#[derive(Serialize, Deserialize, Debug, PartialEq, Eq)]
pub struct BmffHash {
...
impl BmffHash {
    ...
    // The BMFFMerklMaps are stored contiguous in the file.  Break this Vec into
    // groups based on the MerkleMap it matches.
    ...
    */
    pub fn verify_stream_hash(
        ...
        if let Some(mm_vec) = self.merkle() {
            ...
            // check initialization segments (must do here in separate loop since MP4 will
            // consume the reader)
            for mm in mm_vec {
            ...
            if is_fragmented {
                for mm in mm_vec {
                    ...
                    for (index, boxes) in moof_chunks.iter().enumerate() {
                        ...
                        // before box exclusion starts at beginning of file until the start of this
                        // chunk
                ...
                // non-timed media so use iloc (awaiting use case/example since the iloc varies
                // by format)
                return Err(Error::HashMismatch(
        ...
        if let Some(mm_vec) = self.merkle() {
            ...
            for bmff_mm in bmff_merkle {
                    ...
                    .find(|mm| mm.unique_id == bmff_mm.unique_id && mm.local_id == bmff_mm.local_id)
                {
                    ...
                    // check the inithash (for fragmented MP4 with multiple files this is the hash
                    // of the init_segment minus any exclusions)
                    if let Some(init_hash) = &mm.init_hash {
                        let bmff_exclusions = &self.exclusions;
...
impl AssertionBase for BmffHash {
    ...
    // todo: this mechanism needs to change since a struct could support different
    // versions
