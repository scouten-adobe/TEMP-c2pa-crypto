...
use anyhow::Result;
use c2pa_crypto::{settings::load_settings_from_str, Builder, CallbackSigner, Reader, SigningAlg};
use serde_json::json;
...
fn ed_sign(data: &[u8], private_key: &[u8]) -> c2pa_crypto::Result<Vec<u8>> {
    ...
    let signing_key =
        SigningKey::try_from(key_bytes).map_err(|e| c2pa_crypto::Error::OtherError(Box::new(e)))?;
    // Sign the data
